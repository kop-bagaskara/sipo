...
@section('title')
    Edit Email Development Setting
@endsection
@section('css')
    <style>
        .card {
...
@section('content')
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h3 class="text-themecolor">Edit Email Development Setting</h3>
        </div>
            ...
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title mb-0">
                        <i class="mdi mdi-pencil-circle"></i> Form Edit Email Development Setting
                    </h4>
                </div>
                <div class="card-body">
                    <form action="{{ route('development-email-notification-settings.update', $setting->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('process_name') is-invalid @enderror"
                                           id="process_name" name="process_name" value="{{ old('process_name', $setting->process_name) }}"
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('process_code') is-invalid @enderror"
                                           id="process_code" name="process_code" value="{{ old('process_code', $setting->process_code) }}"
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror"
                                      ...
                                      placeholder="Deskripsi singkat tentang proses ini">{{ old('description', $setting->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            <label>Recipient Users <span class="text-danger">*</span></label>
                            <div class="row">
                                @php
                                    $currentUsers = old('recipient_users', $setting->users->pluck('id')->toArray() ?? []);
                                @endphp
                                    ...
                                    <div class="col-md-4 col-lg-3 mb-2">
                                        <div class="form-check">
                                            ...
                                            <label class="form-check-label" for="user_{{ $user->id }}">
                                                {{ $user->name }}
                                                <small class="text-muted d-block">{{ $user->divisiUser->divisi ?? 'N/A' }}</small>
                                            </label>
                                        </div>
                                    </div>
                                @endforeach
                            ...
                            <div class="col-md-6">
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input" id="send_to_rnd_on_every_change"
                                           ...
                                           {{ old('send_to_rnd_on_every_change', $setting->send_to_rnd_on_every_change) ? 'checked' : '' }}>
...
$(document).ready(function() {
    ...
    @php
        $existingReminders = old('reminder_schedule', $setting->reminder_schedule ?? []);
    @endphp

    const existingReminders = @json($existingReminders);
    ...
    if (existingReminders && Object.keys(existingReminders).length > 0) {
        Object.values(existingReminders).forEach(reminder => {
            addReminderCard(reminder);
        });
    } else {
        // Add one default reminder card if no existing data
        addReminderCard();
    }
});
</script>
