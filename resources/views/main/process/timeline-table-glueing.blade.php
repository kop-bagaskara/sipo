            ...
            <div class="card">
                <div class="card-body">
                    <div class="btn-group mb-3" role="group" aria-label="View Toggle">
                        ...
                        <a href="{{ route('process.plan-first-table-glueing') }}" class="btn btn-info">
    ...
    <script>
        ...
        const DataManager = {
            async loadData() {
                try {
                    ...
                    // Untuk GLUEING: gunakan route khusus yang filter process = LEM/GLUEING
                    // Ambil tanggal dari input field jika currentFilters belum di-set
                    const dateFromInput = document.getElementById('date-from');
                    const dateToInput = document.getElementById('date-to');
                    const dateFrom = currentFilters.dateFrom || (dateFromInput ? dateFromInput.value : '');
                    const dateTo = currentFilters.dateTo || (dateToInput ? dateToInput.value : '');

                    const glueingUrl = dateFrom && dateTo
                        ? "{{ route('plan.glueing.data') }}?date_from=" + encodeURIComponent(dateFrom) + "&date_to=" + encodeURIComponent(dateTo)
                        : "{{ route('plan.glueing.data') }}";

                    console.log('🌐 Loading GLUEING data from:', glueingUrl);
                    ...
                    const [machineRes, planRes, deptRes, shiftConfigRes] = await Promise.all([
                        ...
                        fetch(glueingUrl).then(r => r.json()),
                    ...
                    // Untuk GLUEING: pastikan response format benar
                    if (planRes.status === 'success') {
                        timelineData = planRes.data || [];
                        console.log('✅ GLUEING data loaded:', timelineData.length, 'items');
                    } else {
                        console.error('❌ Failed to load GLUEING data:', planRes);
                        timelineData = [];
                    }
                    ...
                    timelineData = timelineData.map(item => {
                        if (!item.process) {
                            item.process = 'LEM'; // Default to LEM for GLUEING timeline
                        }
                    ...
                    // Filter hanya untuk proses GLUEING/LEM
                    timelineData = timelineData.filter(item => {
                        const process = item.process ? item.process.toUpperCase() : '';
                        const isGLUEING = process.includes('LEM') ||
                                         process.includes('GLUEING') ||
                                         process.includes('GLU');
                        return isGLUEING;
                    });

                    console.log('✅ Filtered GLUEING data count:', timelineData.length);
                    console.log('📊 Sample GLUEING data (first 3):', timelineData.slice(0, 3).map(item => ({
                        id: item.id,
            ...
            getFilteredData() {
                ...
                // Process filter - allow GLUEING/LEM and Maintenance items
                filteredData = filteredData.filter(item => {
                    if (item.is_maintenance) {
                        return true; // Always include maintenance items
                    }
                    // Filter untuk GLUEING/LEM
                    const process = item.process ? item.process.toUpperCase() : '';
                    const isGLUEING = process.includes('LEM') ||
                                     process.includes('GLUEING') ||
                                     process.includes('GLU');
        ...
        `;
            ...
            showAddPlanManualModal(machineCode) {
                ...
                document.getElementById('manual-process').value = 'LEM';
            ...
            addPlanManual() {
                try {
                    ...
                    const process = processInput ? processInput.value : 'LEM';
                    ...
                    if (processInput) processInput.value = 'LEM';
            ...
            exportToExcel() {
                ...
                machines.forEach(machineCode => {
                    ...
                    const excelData = [
                        ...
                        [],
                        // Row 5: Headers
                        ['Mesin', 'Nama Order', 'Kode Design', 'Target PCS', 'Up', 'No. WO / JO', 'Catatan', 'Keterangan', 'Delivery']
                    ];
                    ...
                    shifts.forEach(shift => {
                        ...
                        shift.items.forEach(item => {
                            ...
                            const row = [
                                ...
                                targetSheets.toLocaleString('id-ID'), // Target PCS (calculated)
                                up, // Up
                ...
                // Generate filename
                const filename = `TARGET_PRODUKSI_GLUEING_${dayName}_${today.getDate()}-${months[today.getMonth()]}-${today.getFullYear()}.xlsx`;
            ...
            async exportToExcelStyled(selectedDate = null, dateRange = null, numShifts = 3) {
                ...
                try {
                    ...
                    const worksheet = workbook.addWorksheet('TARGET PRODUKSI GLUEING');
                    ...
                    for (let dateChunkStart = 0; dateChunkStart < cetakDates.length; dateChunkStart += DATE_CHUNK_SIZE) {
                        ...
                        for (let d = dateChunkStart; d < dateChunkEnd; d++) {
                            ...
                            titleCell.value = 'TARGET PRODUKSI GLUEING';
                            ...
                            // Column structure: A=Mesin, B=SHIFT, C=Nama Order, D=Kode Design, E=Target PCS, F=Up, G=No. WO, H=Catatan, I=Keterangan, J=Jam, K=Delivery
                            const headers = [
                                { col: 1, value: 'Mesin' },      // A
                                { col: 2, value: 'SHIFT' },     // B
                                { col: 3, value: 'Nama Order' }, // C
                                { col: 4, value: 'Kode Design' }, // D
                                { col: 5, value: 'Target PCS' }, // E
                                { col: 6, value: 'Up' },         // F
                                { col: 7, value: 'No. WO' },    // G
                    ...
                    let filename = `TARGET_PRODUKSI_GLUEING_${dayName}_${today.getDate()}-${months[today.getMonth()]}-${today.getFullYear()}`;
                    if (dateRange) {
                        const startDate = new Date(dateRange.start);
                        const endDate = new Date(dateRange.end);
                        filename = `TARGET_PRODUKSI_GLUEING_${startDate.getDate()}-${months[startDate.getMonth()]}-${startDate.getFullYear()}_sampai_${endDate.getDate()}-${months[endDate.getMonth()]}-${endDate.getFullYear()}`;
                    } else if (selectedDate) {
                        const selDate = new Date(selectedDate);
                        filename = `TARGET_PRODUKSI_GLUEING_${selDate.getDate()}-${months[selDate.getMonth()]}-${selDate.getFullYear()}`;
                    }
                    filename += '.xlsx';
            ...
            async exportToExcelBasic() {
                ...
                try {
                    ...
                    const addMachineDataRows = (machineCode, machineData, isCadangan = false) => {
                        ...
                        groupsToProcess.forEach((group, groupIndex) => {
                            ...
                            shiftData.forEach((item) => {
                                ...
                                // Row data: Mesin, SHIFT, Nama Order, Kode Design, Target PCS, Up, No. WO, Catatan (Job Order), Keterangan (Ukuran Kertas), Delivery, Status
                                ...
                                for (let i = 1; i <= 11; i++) {
                                    ...
                                    } else if (i === 5 || i === 6 || i === 7 || i === 10 || i === 11) { // Target PCS, Up, No. WO, Delivery, Status
                    ...
                    worksheet.columns = [
                        ...
                        { width: 10 }, // E: Target PCS - diperpendek
                        { width: 6 },  // F: Up
                        { width: 18 }, // G: No. WO
                    ...
                    // Generate filename
                    let filename = `TARGET_PRODUKSI_GLUEING_${today.getDate()}-${months[today.getMonth()]}-${today.getFullYear()}.xlsx`;
            ...
            try {
                ...
                const response = await fetch('/sipo/process/get-plan-preview', {
                    ...
                    body: JSON.stringify({
                        date: formattedDate,
                        machine_code: machineCode,
                        process_type: 'GLUEING' // Send process type for filtering
                    })
                });
