        ...
        <script>
            $(document).ready(function() {
                ...
                const table = $('#datatable-work-order').DataTable({
                    ...
                    columns: [{
                            ...
                            render: function(data, type, row) {
                                ...
                                // Return langsung karena backend sudah handle tooltip
                    ...
                    const planningData = {
                        data: selectedRows.map((row, index) => {
                            ...
                            .trim(), // Tipe proses untuk mapping
                                MachineCode: 'CTK-001', // Default machine code
                        ...
                        total_items: Math.max(0, selectedRows.length)
                    };
                        ...
                        .then(async response => {
                            ...
                            if (res.success === false && res.preview === true && res.data) {
                                ...
                                try {
                                    ...
                                    const missingMappingItems = itemsForCheck.filter(it => {
                ...
                async function generateIntegratedProcessTable(filteredItems, startTime, selectedProcess) {
                    ...
                    // 🔧 PERBAIKAN: Gunakan for...of loop untuk support async/await
                    const machineKeys = Object.keys(machineGroups);
                    for (let machineIndex = 0; machineIndex < machineKeys.length; machineIndex++) {
                        const machine = machineKeys[machineIndex];
                        const items = machineGroups[machine];
                        ...
                        // Generate row untuk setiap item dalam mesin yang sama
                        ...
                        if (!Array.isArray(pposSchedules)) {
                            // Jika bukan array, convert ke array
                        ...
                        if (sortedPposSchedules.length > 0) {
                            sortedPposSchedules.forEach((pposSchedule, pposIndex) => {
                                ...
                                const pposDuration = (pposEndDateTime - pposStartDateTime) / (1000 * 60 * 60); // dalam jam
                                ...
                                // Update currentTime untuk item berikutnya (gunakan PPOS terakhir)
                                if (pposIndex === sortedPposSchedules.length - 1) {
                                    currentTime = new Date(pposEndDateTime);
                                    totalProcessTime += pposDuration;
                        ...
                        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
                            ...
                            if (item.WODocNo && item.WODocNo.startsWith('WOP')) {
                                ...
                                if (item.estHours && item.estHours > 0) {
                                    ...
                                    try {
                                        ...
                                        // console.log('machineData Data', machineData);
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    // console.log('11 machineUnit', data);
                                }
                            })


                            // // get data unit
                            // $.getJSON('{{ route('get-machine-capacity') }}', {
                            //     item: item
                            // }, function(data) {
                            //     console.log('11 dataUnit', data);
                            //     var dataUnit = data.unit;
                            // });

                            console.log('11 machineUnit', machineUnit);
                            console.log('11 item.Unit', item.Unit);
                            console.log('11 item.Up', item.UP);
                            console.log('11 displayQuantity', displayQuantity);

                            // REMOVED: Jangan bagi displayQuantity dengan UP
                            // UP hanya digunakan untuk konversi unit saat perhitungan estimasi waktu,
                            // bukan untuk display quantity. Quantity yang ditampilkan harus quantity asli.
                            // if (machineUnit !== item.Unit) {
                            //     displayQuantity = parseFloat(displayQuantity) / parseFloat(item.plans?.[0]?.UP);
                            // }




                            // Cek apakah item ini punya mapping mesin
                            ...
                            html += `
                                <tr class="${rowClass}">
                                    <td class="text-center" id="row-${machineIndex + 1}-${itemIndex + 1}-no"><strong>${itemIndex + 1}</strong></td>
                                    <td id="row-${machineIndex + 1}-${itemIndex + 1}-wodocno"><strong>${item.WODocNo || '-'}</strong></td>
                                    <td id="row-${machineIndex + 1}-${itemIndex + 1}-materialcode">${item.CodeItem || '-'}</td>
                                    <td id="row-${machineIndex + 1}-${itemIndex + 1}-materialname">${item.MaterialName || '-'}</td>
                                    <td class="text-center" id="row-${machineIndex + 1}-${itemIndex + 1}-quantity">${displayQuantity}</td>
                                    <td class="text-success font-weight-bold" id="row-${machineIndex + 1}-${itemIndex + 1}-starttime">${startTimeFormatted}</td>
                                    <td class="text-danger font-weight-bold" id="row-${machineIndex + 1}-${itemIndex + 1}-endtime">${endTimeFormatted}</td>
                                    <td class="text-center" id="row-${machineIndex + 1}-${itemIndex + 1}-duration">${estHours.toFixed(2)} jam</td>
                                    <td class="text-center" id="row-${machineIndex + 1}-${itemIndex + 1}-up">${item.Up || item.plans?.[0]?.UP || '1'}</td>
                                    <td class="text-center" id="row-${machineIndex + 1}-${itemIndex + 1}-status">${statusDisplay}</td>
                                </tr>
                            `;
                            ...
                            console.log(`  Total process time so far: ${totalProcessTime} hours`);
                            console.log('---');
                        } // 🔧 PERBAIKAN: Tutup for loop
                        ...
                        html += `
                                ...
                                <div class="mt-3">
                                    <div class="alert alert-info">
                                        ...
                                        <p class="mb-1"><strong>Total Duration:</strong> ${totalProcessTime.toFixed(2)} jam</p>
                    ...
                    } // 🔧 PERBAIKAN: Tutup for loop machine
