...
@section('title')
    Plan Production - Timeline (Table View PLONG)
@endsection
@section('css')
...
@section('content')
            ...
            <div class="card">
                <div class="card-body">
                    <div class="btn-group mb-3" role="group" aria-label="View Toggle">
                        ...
                        <a href="{{ route('process.plan-first-table') }}" class="btn btn-outline-info">
                            <i class="mdi mdi-widgets"></i> PLANNING CETAK
                        </a>
                        <a href="{{ route('process.plan-first-table-plong') }}" class="btn btn-info">
                    ...
                    <div class="filter-section">
                        ...
                        <div class="filter-actions">
                            ...
                            <div class="btn-group ml-2" role="group">
                                <button type="button" class="btn btn-outline-secondary" id="view-history-btn" title="Lihat History Perubahan Plan">
                                    <i class="mdi mdi-history"></i> View History
                            </button>
                                <button type="button" class="btn btn-outline-primary" id="compare-versions-btn" title="Bandingkan Versi Plan">
                    ...
                    <div class="export-section">
                        ...
                        <div class="export-buttons">
                            <button type="button" class="export-btn btn-excel" id="export-excel-direct">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-warning">
                        <i class="mdi mdi-information-outline"></i>
                        <small><strong>Catatan:</strong> Mulai dan Akhir Proses akan dihitung otomatis berdasarkan plan terakhir di mesin ini. Plan manual akan ditambahkan di urutan paling akhir (prioritas terakhir) dan bisa diubah prioritasnya nanti.</small>
                    </div>
    ...
    <div class="modal fade" id="moveItemModal" tabindex="-1" role="dialog" aria-labelledby="moveItemModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header bg-info">
                    <h5 class="modal-title" id="moveItemModalLabel" style="color: white;">
...
@section('scripts')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Fallback: Setup apply filter button dengan event delegation (untuk memastikan ter-attach)
            // Gunakan event delegation untuk memastikan event handler selalu ter-attach
            document.addEventListener('click', function(e) {
                const applyBtn = e.target.closest('#apply-filters');
                if (applyBtn) {
                    e.preventDefault();
                    e.stopPropagation();
                    console.log('🔘 Apply Filter clicked (event delegation)!');
                    ...
                    if (!dateFrom || !dateTo) {
                        Utils.showToast('Silakan isi "Dari Tanggal" dan "Sampai Tanggal" terlebih dahulu', 'warning');
                        return;
                    }
                    ...
                    // Load data
                    console.log('🔄 Calling loadPlanData()...');
                    DataManager.loadPlanData();
                }
            });

            // Juga setup langsung untuk memastikan
            setTimeout(function() {
                const applyBtn = document.getElementById('apply-filters');
                if (applyBtn) {
                    console.log('✅ Apply filter button found, setting up direct listener');
                    applyBtn.addEventListener('click', function(e) {
                        e.preventDefault();
                        e.stopPropagation();
                        console.log('🔘 Apply Filter clicked (direct listener)!');
                    ...
                    console.error('❌ Apply filter button not found in DOM!');
                }
            }, 500);
        ...
        const DataManager = {
            async loadData() {
                try {
                    ...
                    // Load hanya machines, departments, dan machine shift config (TIDAK load plan data)
                    const [machineRes, deptRes, shiftConfigRes] = await Promise.all([
                    ...
                    timelineData = []; // Kosongkan data - hanya akan di-load setelah filter diterapkan
                    ...
                    Utils.showToast('Silakan isi filter tanggal dan klik "Terapkan Filter" untuk melihat data', 'info');
            ...
            async loadPlanData() {
                try {
                    ...
                    console.log('📅 Current filters before load:', currentFilters);

                    // Pastikan filter tanggal sudah di-update
                    if (!currentFilters.dateFrom || !currentFilters.dateTo) {
                        console.error('❌ Date filters are empty!');
                        Utils.showToast('Silakan isi "Dari Tanggal" dan "Sampai Tanggal" terlebih dahulu', 'warning');
                        DataManager.showLoading(false);
                        return;
                    }

                    // Load plan data dari server dengan parameter tanggal (khusus PLONG)
                    const url = "{{ route('plan.plong.data') }}?date_from=" + encodeURIComponent(currentFilters.dateFrom) + "&date_to=" + encodeURIComponent(currentFilters.dateTo);
                    console.log('🌐 Request URL (PLONG):', url);
                    ...
                    const planRes = await response.json();
                    console.log('📥 Plan data response:', planRes);

                    if (planRes.status !== 'success') {
                        throw new Error('Server returned error: ' + (planRes.message || 'Unknown error'));
                    }

                    let loadedData = planRes.data || [];
                    console.log('📊 Loaded data count:', loadedData.length);
                    console.log('📊 Loaded data details (first 5):', loadedData.slice(0, 5).map(item => ({
                        id: item.id,
                    ...
                    // Debug: Cek item dengan code_item dan wo_docno yang sama
                    const duplicateCheck = loadedData.filter(item =>
                        item.code_item === 'DS.0190.0260' && item.wo_docno === 'WOT-250613-0003'
                    );
                    console.log('🔍 Duplicate check for DS.0190.0260 / WOT-250613-0003:', duplicateCheck.length, 'items found');
                    console.log('🔍 Duplicate items details:', duplicateCheck.map(item => ({
                        id: item.id,
                        code_item: item.code_item,
                        wo_docno: item.wo_docno,
                        code_item_bom: item.code_item_bom,
                    ...
                    // Debug: Cek semua item tanpa machine
                    const itemsWithoutMachineFromBackend = loadedData.filter(item =>
                        !item.code_machine || item.code_machine === '' || item.code_machine === null
                    );
                    console.log('🔍 Items without machine from backend:', itemsWithoutMachineFromBackend.length);
                    console.log('🔍 Items without machine details:', itemsWithoutMachineFromBackend.map(item => ({
                        id: item.id,
                        code_item: item.code_item,
                        wo_docno: item.wo_docno,
                        code_item_bom: item.code_item_bom,
                    ...
                    if (loadedData.length === 0) {
                        ...
                        timelineData = [];
                        TableRenderer.renderTable();
                        Utils.showToast('Tidak ada data untuk periode yang dipilih', 'info');
                        DataManager.showLoading(false);
                        return;
                    }

                    timelineData = loadedData.map(item => {
                        if (!item.process) {
                            item.process = 'PLG';
                        }
                        return item;
                    ...
                    // Filter hanya untuk proses PLONG (PLG)
                    timelineData = timelineData.filter(item => {
                        const isPLG = item.process && (item.process.toUpperCase().includes('PLG') || item.process.toUpperCase().includes('PLONG'));
                        return isPLG;
                    });

                    console.log('✅ Filtered PLONG data count:', timelineData.length);
                    ...
                    // Render table setelah data di-load
                    console.log('🎨 Rendering table...');
                    TableRenderer.renderTable();

                    // Get filtered data untuk menampilkan jumlah yang benar
                    const filteredData = DataManager.getFilteredData();
                    console.log('📋 Filtered data count after render:', filteredData.length);

                    Utils.showToast(`Data berhasil dimuat: ${filteredData.length} item ditampilkan`, 'success');
            ...
            populateFilters() {
                ...
                // Kosongkan field tanggal - user yang akan mengisi
            ...
            getFilteredData() {
                ...
                console.log('🔍 getFilteredData - Initial data count:', filteredData.length);
                console.log('🔍 Current filters:', currentFilters);

                // Debug: Cek item duplikat sebelum filter
                const duplicateBeforeFilter = filteredData.filter(item =>
                    item.code_item === 'DS.0190.0260' && item.wo_docno === 'WOT-250613-0003'
                );
                console.log('🔍 Duplicate items BEFORE filter:', duplicateBeforeFilter.length);
                console.log('🔍 Duplicate items BEFORE filter details:', duplicateBeforeFilter.map(item => ({
                    id: item.id,
                    code_item: item.code_item,
                    wo_docno: item.wo_docno,
                    code_item_bom: item.code_item_bom,
                ...
                // Process filter - allow PLG/PLONG and Maintenance items
                filteredData = filteredData.filter(item => {
                    if (item.is_maintenance) {
                        return true; // Always include maintenance items
                    }
                    if (!item.process || (!item.process.toUpperCase().includes('PLG') && !item.process.toUpperCase().includes('PLONG'))) {
                        return false;
                    }
                ...
                console.log('🔍 After process filter:', filteredData.length);
                ...
                if (currentFilters.dateFrom && currentFilters.dateTo) {
                    ...
                    filteredData = filteredData.filter(item => {
                        ...
                        const matches = itemDate >= currentFilters.dateFrom && itemDate <= currentFilters.dateTo;
                        if (!matches) {
                            console.log(`❌ Item ${item.code_item} date ${itemDate} tidak sesuai filter ${currentFilters.dateFrom} - ${currentFilters.dateTo}`);
                        }
                        return matches;
                    });
                    console.log('🔍 After date filter:', filteredData.length, '(was', beforeDateFilter + ')');
                } else {
                    console.log('⚠️ No date filter applied - dateFrom or dateTo is empty');
                }
            ...
            moveItemToMachine(itemId, targetMachine, moveQuantity = null) {
                return new Promise((resolve) => {
                    try {
                        ...
                        // Get target machine data untuk update capacity
                        const targetMachineData = allMachines.find(m => m.Code === targetMachine);
                        const capacityPerHour = targetMachineData ? parseFloat(targetMachineData.CapacityPerHour) || 10000 : 10000;

                        if (moveQuantity >= item.quantity) {
                            item.code_machine = targetMachine;
                            // Update capacity sesuai dengan mesin target
                            item.capacity = capacityPerHour;
                            console.log(`📊 Updated capacity to ${capacityPerHour} (from machine ${targetMachine})`);
                        } else {
                            ...
                            const newItem = {
                                ...
                                capacity: capacityPerHour // Update capacity sesuai dengan mesin target
                            };

                            // Add new item to timeline data
                            timelineData.push(newItem);
                            console.log(`📊 New item capacity set to ${capacityPerHour} (from machine ${targetMachine})`);
                        }
                        ...
                        // targetMachineData dan capacityPerHour sudah dideklarasikan di atas
                        if (targetMachineData) {
                            ...
                            if (moveQuantity >= item.quantity) {
                                ...
                                // Capacity sudah di-update di atas, hanya perlu update timing
                                ...
                                // Capacity sudah di-set saat membuat newItem di atas, hanya perlu update timing
        ...
        const Utils = {
            ...
            // Format tanggal dan waktu untuk kolom MULAI PROSES dan AKHIR PROSES
        ...
        const EventManager = {
            ...
            setupFilterEvents() {
                ...
                // Apply filters button - setup di EventManager juga
                const applyBtn = document.getElementById('apply-filters');
                if (applyBtn) {
                    // Remove existing listener jika ada dengan clone
                    ...
                    newApplyBtn.addEventListener('click', function(e) {
                        e.preventDefault();
                        e.stopPropagation();
                        console.log('🔘 Apply Filter button clicked (from EventManager)!');

                        // Validasi bahwa tanggal sudah diisi
                        const dateFrom = document.getElementById('date-from').value;
                        const dateTo = document.getElementById('date-to').value;
                        console.log('📅 Date inputs - From:', dateFrom, 'To:', dateTo);
                        ...
                        // Load data plan setelah filter diterapkan
                        console.log('🔄 Calling loadPlanData()...');
                        DataManager.loadPlanData();
                    });
                    console.log('✅ Apply filter event listener attached (EventManager)');
                }
            },
        ...
        const TableRenderer = {
            ...
            generateMachineTables(data) {
                console.log('🔍 generateMachineTables - Total data received:', data.length);
                console.log('🔍 generateMachineTables - Sample data:', data.slice(0, 3).map(item => ({
                    id: item.id,
                    code_item: item.code_item,
                    wo_docno: item.wo_docno,
                    code_item_bom: item.code_item_bom,
                ...
                // Pisahkan item yang code_machine kosong/null (untuk tabel global)
                const itemsWithoutMachine = data.filter(item => !item.code_machine || item.code_machine === '' || item.code_machine === null);
                const itemsWithMachine = data.filter(item => item.code_machine && item.code_machine !== '' && item.code_machine !== null);

                console.log('🔍 Items without machine count:', itemsWithoutMachine.length);
                console.log('🔍 Items without machine details:', itemsWithoutMachine.map(item => ({
                    id: item.id,
                    code_item: item.code_item,
                    wo_docno: item.wo_docno,
                    code_item_bom: item.code_item_bom,
                ...
                // Ambil mesin yang ada (hanya yang punya code_machine)
                const machines = [...new Set(itemsWithMachine.map(item => item.code_machine))];
                ...
                // TAMBAHAN: Render tabel global untuk item yang belum ada mesinnya
                if (itemsWithoutMachine.length > 0) {
                    const globalContainer = document.createElement('div');
                    globalContainer.className = 'machine-table-container';
                    globalContainer.id = 'machine-global';
                    globalContainer.style.border = '2px solid #007bff'; // Border biru untuk membedakan

                    globalContainer.innerHTML = `
                        <div class="machine-header d-flex justify-content-between align-items-center" style="background: linear-gradient(135deg, #007bff 0%, #0056b3 100%); color: white; padding: 15px; border-radius: 8px;">
                            <h4 class="mb-0" style="color: white;">
                                <i class="mdi mdi-alert-circle"></i> Tabel Global
                                <span class="badge badge-light ml-2" style="background: rgba(255,255,255,0.3); color: white;">${itemsWithoutMachine.length} Item</span>
                            </h4>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-sm table-bordered machine-table" id="machine-table-global">
                                <thead>
                                    <tr style="white-space: nowrap;">
                                        <th class="text-center" style="width: 80px;">PRIORITY</th>
                                        <th>KODE ITEM</th>
                                        <th>NAMA ITEM</th>
                                        <th>MATERIAL CODE BOM</th>
                                        <th>WO DOCNO</th>
                                        <th>QTY ORDER</th>
                                        <th>TGL DELIVERY</th>
                                        <th>PROCESS</th>
                                        ...
                                        <th>MULAI PROSES</th>
                                        <th>AKHIR PROSES</th>
                                        ${currentFilters.dateFrom && currentFilters.dateTo ? '<th>CATATAN</th>' : ''}
                                        <th>AKSI</th>
                                    </tr>
                                </thead>
                                <tbody id="machine-tbody-global">
                                    <!-- Global data (items without machine) will be loaded here -->
                                </tbody>
                            </table>
                        </div>
                    `;
                    ...
                    // Populate tabel global
                    this.populateMachineTable('global', itemsWithoutMachine);
                }
                ...
                const renderMachineBatch = () => {
                    ...
                    // Gunakan itemsWithMachine (bukan data) karena sudah difilter sebelumnya
                    const machineData = itemsWithMachine
                        .filter(item => item.code_machine === machineCode)
                        .sort((a, b) => {
            ...
            populateMachineTable(machineCode, machineData) {
                ...
                // Handle case untuk tabel global (machineCode = 'global')
                const tbodyId = machineCode === 'global' ? 'machine-tbody-global' : `machine-tbody-${machineCode}`;
                ...
                if (machineData.length === 0) {
                    const message = machineCode === 'global' ? 'Tidak ada item yang belum memiliki mesin' : `Tidak ada data untuk mesin ${machineCode}`;
                    tbody.innerHTML = `
                        <tr style="white-space: nowrap;">
                ...
                // Untuk tabel global, tidak perlu shift config (default 3)
                const numShifts = machineCode === 'global' ? 3 : (machineShiftConfig[machineCode] || 3);
                    ...
                    row.innerHTML = `
                        ...
                        <td>
                            ${(() => {
                                ...
                                // Buat input datetime-local untuk edit tanggal mulai proses
                                ...
                                if (!item.start_jam) {
                                    ...
                                    return `
                                        <input type="datetime-local"
                                               ...
                                               title="Edit Tanggal & Waktu Mulai Proses">
                                    `;
                                }
                        ...
                        <td>
                            <select class="form-control form-control-sm catatan-select"
                                    data-item-id="${item.id}"
                                    data-machine="${item.code_machine}"
                                    style="width: 100%; max-width: 150px; font-size: 0.85rem;"
                                    title="Pilih Catatan Proses">
                                <option value="">-- Pilih --</option>
                                <option value="UV" ${item.catatan === 'UV' ? 'selected' : ''}>UV</option>
                                <option value="EMBOSS" ${item.catatan === 'EMBOSS' ? 'selected' : ''}>EMBOSS</option>
                                <option value="HOTPRINT" ${item.catatan === 'HOTPRINT' ? 'selected' : ''}>HOTPRINT</option>
                                <option value="PLONG" ${item.catatan === 'PLONG' ? 'selected' : ''}>PLONG</option>
                            </select>
                        </td>
                        ` : ''}
                        <td>
            ...
            showMoveItemModal(itemId) {
                ...
                // Filter hanya mesin PLG dari department yang sama
                const targetSelect = document.getElementById('target-machine-select');
                targetSelect.innerHTML = '<option value="">-- Pilih Mesin --</option>';

                // Cari mesin item saat ini untuk mendapatkan department
                const currentMachine = allMachines.find(m => m.Code === item.code_machine);
                const currentDepartment = currentMachine ? currentMachine.Department : null;

                allMachines.forEach(machine => {
                    // Filter: hanya mesin PLG, bukan mesin saat ini, dan dari department yang sama
                    const isPLG = machine.Code && machine.Department.toUpperCase().startsWith('PLG');
                    const isDifferentMachine = machine.Code !== item.code_machine;
                    const isSameDepartment = !currentDepartment || machine.Department === currentDepartment;

                    if (isPLG && isDifferentMachine && isSameDepartment) {
                        const option = document.createElement('option');
                        option.value = machine.Code;
                        option.textContent = `${machine.Code} - ${machine.Description}`;
        ...
        `;
                ...
                allMachines.forEach(machine => {
                    // Filter: hanya mesin PLG, bukan mesin saat ini, dan dari department yang sama
                    const isPLG = machine.Code && machine.Code.toUpperCase().startsWith('PLG');
                    const isDifferentMachine = machine.Code !== item.code_machine;
                    const isSameDepartment = !currentDepartment || machine.Department === currentDepartment;
            ...
            showAddPlanManualModal(machineCode) {
                ...
                const machineName = machine.Description ;
                ...
                document.getElementById('manual-process').value = 'PLG';
            ...
            addPlanManual() {
                try {
                    ...
                    const process = processInput ? processInput.value : 'PLG';
            ...
            exportToExcel() {
                ...
                machines.forEach(machineCode => {
                    ...
                    shifts.forEach(shift => {
                        ...
                        const machineRow = Array(9).fill('');
                        machineRow[0] = machineName;
                        excelData.push(machineRow);

                        // Add shift header
                        const shiftRow = Array(9).fill('');
                        ...
                        shift.items.forEach(item => {
                            ...
                            const row = [
                                ...
                                item.process || '-', // Catatan
                                `${startTime} s/d ${endTime}`, // Keterangan (waktu cetak)
            ...
            async exportToExcelStyled(selectedDate = null, dateRange = null, numShifts = 3) {
                ...
                try {
                    ...
                    const worksheet = workbook.addWorksheet('TARGET PRODUKSI PLONG');
                    ...
                    for (let dateChunkStart = 0; dateChunkStart < cetakDates.length; dateChunkStart += DATE_CHUNK_SIZE) {
                        ...
                        for (let d = dateChunkStart; d < dateChunkEnd; d++) {
                            ...
                            titleCell.value = 'TARGET PRODUKSI PLONG';
            ...
            async exportToExcelBasic() {
                ...
                try {
                    ...
                    // Row 1: Title - "TARGET PRODUKSI CETAK 19" - merged A1:L1 (kolom J dan K dihapus, tapi kolom L masih digunakan)
                    ...
                    // Kolom J dan K dihapus, jadi tanggal langsung di kolom L
                    const dateCell = worksheet.getCell(`L${rowNum}`);
                    ...
                    const headers = [
                        ...
                        { col: 8, value: 'Catatan' },     // H
                        { col: 9, value: 'Keterangan' },   // I
                        // { col: 10, value: 'Start Cetak' }, // J - DISABLED: Kolom Start Cetak di-comment
                        // { col: 11, value: 'End Cetak' },   // K - DISABLED: Kolom End Cetak di-comment
                        { col: 12, value: 'Delivery' }    // L
                    ];
                    ...
                    const addMachineDataRows = (machineCode, machineData, isCadangan = false) => {
                        ...
                        // Add data rows grouped by shift
                        shiftNames.forEach((shift, shiftIndex) => {
                            const shiftData = shiftGroups[shift];
                            ...
                            shiftData.forEach((item) => {
                                ...
                                // Row data: Mesin, SHIFT, Nama Order, Kode Design, Target Sheet, Up, No. WO, Catatan, Keterangan, Delivery
                                ...
                                row.getCell(8).value = ''; // Catatan - kosong
                                row.getCell(9).value = item.process || '-';
                                // DISABLED: Start Cetak dan End Cetak di-comment
                                // row.getCell(10).value = startCetak;
                                // row.getCell(11).value = endCetak;
                                row.getCell(12).value = deliveryDate;

                                // Apply borders to all cells (kolom 1-9 dan 12, skip 10 dan 11 karena di-comment)
                                for (let i = 1; i <= 9; i++) {
                                    const cell = row.getCell(i);
                                    cell.border = dottedBorder;
                                }
                                // Kolom 12 (Delivery)
                                row.getCell(12).border = dottedBorder;
                    ...
                    const todayMachineGroups = sortAndGroupData(todayData);
                    const cadanganMachineGroups = sortAndGroupData(cadanganData);
                    ...
                    allMachineCodes.forEach((machineCode, machineIndex) => {
                        ...
                        if (cadanganMachineData.length > 0) {
                            // Add CADANGAN header row (merged)
                    ...
                    // Set column widths (kolom J dan K dihapus)
                    worksheet.columns = [
                        { width: 8 }, // A: Mesin
                        { width: 9 }, // B: SHIFT
                        { width: 50 }, // C: Nama Order
                ...
                items.forEach((item, index) => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>
                            <select class="form-control form-control-sm catatan-select"
                                    data-item-id="${item.id}"
                                    data-machine="${item.code_machine || machineCode}"
                                    style="width: 100%; max-width: 150px; font-size: 0.85rem;"
                                    title="Pilih Catatan Proses">
                                <option value="">-- Pilih --</option>
                                <option value="UV" ${item.catatan === 'UV' ? 'selected' : ''}>UV</option>
            ...
            showFinishJobModal(itemId) {
                ...
                // Add mesin yang sama dulu sebagai default (jika PLG)
                if (currentMachine && currentMachine.Code && currentMachine.Code.toUpperCase().startsWith('PLG')) {
                ...
                // Add mesin PLG lainnya dari department yang sama
                allMachines.forEach(machine => {
                    const isPLG = machine.Code && machine.Code.toUpperCase().startsWith('PLG');
                    const isDifferentMachine = machine.Code !== item.code_machine;
                    const isSameDepartment = !currentDepartment || machine.Department === currentDepartment;
        ...
        const EventHandlers = {
            ...
            setupFilterEvents() {
                ...
                const applyBtn = document.getElementById('apply-filters');
                console.log('🔍 Setting up filter events, apply button found:', !!applyBtn);
                ...
                if (applyBtn) {
                    ...
                    newApplyBtn.addEventListener('click', function(e) {
                        ...
                        // Update filters terlebih dahulu
                        EventHandlers.updateFilters();
                        console.log('✅ Filters updated:', currentFilters);
                    ...
                    console.log('✅ Apply filter event listener attached');
                } else {
            ...
            updateFilters() {
                const dateFromInput = document.getElementById('date-from');
                const dateToInput = document.getElementById('date-to');
                const machineInput = document.getElementById('machine-filter');
                const departmentInput = document.getElementById('department-filter');

                currentFilters.dateFrom = dateFromInput ? dateFromInput.value : '';
                currentFilters.dateTo = dateToInput ? dateToInput.value : '';
                currentFilters.machine = machineInput ? machineInput.value : '';
                currentFilters.department = departmentInput ? departmentInput.value : '';

                console.log('✅ Filters updated:', currentFilters);
                console.log('📅 Date inputs - From:', dateFromInput?.value, 'To:', dateToInput?.value);
            },
            ...
            resetFilters() {
                ...
                // Reset currentFilters - kosongkan semua filter
                currentFilters.dateFrom = '';
                currentFilters.dateTo = '';
                currentFilters.machine = '';
                currentFilters.department = '';

                // Kosongkan timelineData dan render ulang tabel
                timelineData = [];
                TableRenderer.renderTable();
        ...
        function preparePlanDataForSave() {
            ...
            timelineData.forEach(item => {
                ...
                const itemData = {
                    ...
                    catatan_proses: item.catatan || null,
        ...
        function setupTimeInputListeners() {
            ...
            // Event listener untuk dropdown catatan - update di timelineData
            document.addEventListener('change', function(e) {
                if (e.target.classList.contains('catatan-select')) {
                    const itemId = e.target.dataset.itemId;
                    const newCatatan = e.target.value;

                    // Update di timelineData
                    const item = timelineData.find(i => i.id == itemId);
                    if (item) {
                        item.catatan = newCatatan;
                        console.log('✅ Catatan updated for item:', item.code_item, '→', newCatatan);
                    }
                }
            });
        }
        ...
        function recalculateMachineTimeline(changedInput) {
            ...
            rows.forEach((row, index) => {
                ...
                const startTimeCell = row.querySelector('td:nth-child(12)'); // MULAI PROSES
                const endTimeCell = row.querySelector('td:nth-child(13)'); // AKHIR PROSES
                const durationCell = row.querySelector('td:nth-child(9)'); // LAMA CETAK
            ...
            items.forEach((item, index) => {
                ...
                // Update tampilan
                // Pastikan kolom MULAI PROSES tetap berupa input datetime-local agar bisa diedit lagi
        ...
        // Fungsi untuk menangani perubahan tanggal mulai proses
        function handleStartDateChange(input) {
            ...
            // Show notification
            Utils.showToast(
                `✅ Tanggal mulai proses ${item.code_item} diupdate: ${Utils.formatDateTime(oldStartDate ? oldStartDate.toISOString() : '')} → ${Utils.formatDateTime(newStartDate.toISOString())}`,
                'success'
            );
