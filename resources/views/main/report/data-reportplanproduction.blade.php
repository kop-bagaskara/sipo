    ...
    @section('content')
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h3 class="text-themecolor">Report Plan Production</h3>
            </div>
        ...
        <div class="row">
            <div class="col">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">Report Plan Production</h5>
                        <hr>
        ...
        <script>
            $(document).ready(function() {
                ...
                function loadWorkOrderPercentageData() {
                    ...
                    $.ajax({
                        url: "{{ route('report.plan-production.data') }}",
                        type: "POST",
                        data: {
                ...
                function renderTableData(data) {
                    ...
                    data.forEach(function(item, index) {
                        ...
                        html += '<td>' + (item.code_item || '-') + '</td>';
                        html += '<td>' + (item.material_name || '-') + '</td>';
                        html += '<td>' + (item.code_machine || '-') + '</td>';
                        html += '<td>' + (item.wo_docno || '-') + '</td>';
                        html += '<td>' + (item.so_docno || '-') + '</td>';
                        html += '<td>' + (item.quantity ? parseFloat(item.quantity).toLocaleString('id-ID') : '0') + '</td>';
                        html += '<td>' + (item.start_jam ? moment(item.start_jam).format('DD-MM-YYYY') : '-') + '</td>';
                        html += '<td>' + (item.end_jam ? moment(item.end_jam).format('DD-MM-YYYY') : '-') + '</td>';
                        html += '<td>' + (item.flag_status || '-') + '</td>';
                        html += '</tr>';
                        rowIndex++;
                    });
                ...
                function exportWorkOrderPercentageToExcel() {
                    ...
                    var form = $('<form>', {
                        'method': 'POST',
                        'action': "{{ route('report.plan-production.export') }}"
                    });
