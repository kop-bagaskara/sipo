                        ...
                        <div class="card-header">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <a href="{{ route('hr.employee-data.import') }}" class="btn btn-success">
                                        <i class="mdi mdi-upload me-2"></i>Import Excel
                                    </a>
                                    <a href="{{ route('hr.employee-data.create') }}" class="btn btn-primary">
    ...
    <script>
        $(document).ready(function() {
            var table = $('#employeeDataTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('hr.employee-data.data') }}",
                    type: "GET"
                },
                columns: [
            ...
            function showEmployeeDetail(employeeId) {
                ...
                $.ajax({
                    url: "{{ route('hr.employee-data.detail', ':id') }}".replace(':id', id),
                    type: 'GET',
                    success: function(response) {
                        if (response.success && response.data) {
                            ...
                            $('#editEmployeeBtn').attr('href', "{{ route('hr.employee-data.edit', ':id') }}".replace(':id', id));
                        } else {
                            $('#employeeDetailContent').html(`
            ...
            function renderEmployeeDetail(emp) {
                const foto = emp.foto_path
                    ? `<img src="{{ asset('storage') }}/${emp.foto_path}" alt="Foto" class="img-fluid rounded" style="max-width: 200px; max-height: 200px; object-fit: cover;">`
