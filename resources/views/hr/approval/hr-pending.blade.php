                                ...
                                <ul class="list-unstyled">
                                                ...
                                                <div class="mt-2">
                                                    ...
                                                    <ol class="mb-0" style="padding-left: 20px;">
                                                        @foreach($approvalInfo['absence_approval_order'] as $orderItem)
                                                            <li class="mb-1">
                                                                <span class="badge badge-{{ $orderItem['is_current_user'] ? 'success' : 'primary' }}">
                                    ...
                                    <li class="mt-2">
                                        ...
                                        @if($approvalInfo['can_approve_shift_change'])
                                            <span class="badge badge-success">Bisa Approve</span>
                                            @if(!empty($approvalInfo['shift_change_approval_order']))
                                                <div class="mt-2">
                                                    <small class="text-muted d-block mb-1"><strong>Urutan Approval Global:</strong></small>
                                                    <ol class="mb-0" style="padding-left: 20px;">
                                                        @foreach($approvalInfo['shift_change_approval_order'] as $orderItem)
                                                            <li class="mb-1">
                                                                <span class="badge badge-{{ $orderItem['is_current_user'] ? 'success' : 'primary' }}">
                                                                    Urutan {{ $orderItem['order'] }}: {{ $orderItem['role'] }}
                                                                    @if($orderItem['is_current_user'])
                                                        ...
                                                        <td>
                                                            @if($request->employee && $request->employee->divisiUser)
                                                                <span class="badge badge-secondary">
                                                                    {{ $request->employee->divisiUser->divisi ?? $request->employee->divisiUser->nama_divisi ?? 'Divisi ' . $request->employee->divisi }}
                                                                </span>
