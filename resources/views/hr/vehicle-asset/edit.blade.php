...
@section('title')
    Edit Pengajuan {{ $type === 'vehicle' ? 'Kendaraan' : 'Inventaris' }}
@endsection
@section('css')
            ...
            <div class="card shadow-sm">
                <div class="card-header text-white @if($type === 'vehicle') bg-info @else bg-success @endif text-white">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <h4 class="card-title mb-0 text-white"><i class="mdi @if($type === 'vehicle') mdi-car @else mdi-package-variant @endif me-2"></i>Edit Pengajuan {{ $type === 'vehicle' ? 'Kendaraan' : 'Inventaris' }}</h4>
                            <p class="card-subtitle mb-0 text-white-50 small mt-1">Edit data pengajuan {{ $type === 'vehicle' ? 'kendaraan' : 'inventaris' }}</p>
                        </div>
                        <div class="text-end">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('hr.vehicle-asset.update', $request->id) }}" id="vehicleAssetForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row mb-3">
                                ...
                                <select name="{{ $type === 'vehicle' ? 'vehicle_type' : 'asset_category' }}"
                                    ...
                                    @if($type === 'vehicle')
                                        <optgroup label="🚗 Kendaraan">
                                            <option value="Mobil" {{ old('vehicle_type', $request->vehicle_type) == 'Mobil' ? 'selected' : '' }}>Mobil</option>
                                            <option value="Motor" {{ old('vehicle_type', $request->vehicle_type) == 'Motor' ? 'selected' : '' }}>Motor</option>
                                        </optgroup>
                                    @else
                                        <optgroup label="💻 Inventaris">
                                            <option value="Laptop" {{ old('asset_category', $request->asset_category) == 'Laptop' ? 'selected' : '' }}>Laptop Company</option>
                                            <option value="Lain-Lain" {{ old('asset_category', $request->asset_category) == 'Lain-Lain' ? 'selected' : '' }}>Inventaris Lain-lain</option>
                                        </optgroup>
                                    @endif
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="purpose_type" class="form-select form-control @error('purpose_type') is-invalid @enderror" required>
                                    ...
                                    <option value="Meeting" {{ old('purpose_type', $request->purpose_type) == 'Meeting' ? 'selected' : '' }}>📅 Meeting</option>
                                    <option value="Dinas Luar" {{ old('purpose_type', $request->purpose_type) == 'Dinas Luar' ? 'selected' : '' }}>🏢 Dinas Luar</option>
                                    <option value="Training" {{ old('purpose_type', $request->purpose_type) == 'Training' ? 'selected' : '' }}>📚 Training</option>
                                    <option value="Presentasi" {{ old('purpose_type', $request->purpose_type) == 'Presentasi' ? 'selected' : '' }}>📊 Presentasi</option>
                                    <option value="Lainnya" {{ old('purpose_type', $request->purpose_type) == 'Lainnya' ? 'selected' : '' }}>📋 Lainnya</option>
                                </select>
                                @error('purpose_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row mb-3">
                            <div class="col-12">
                                ...
                                <textarea name="purpose" class="form-control @error('purpose') is-invalid @enderror" rows="3"
                                          placeholder="Contoh: @if($type === 'vehicle') Menghadiri meeting dengan client PT. XYZ di Jakarta untuk presentasi produk. @else Mengikuti training di luar kota dan membutuh laptop untuk presentasi. @endif" required>{{ old('purpose', $request->purpose) }}</textarea>
                                @error('purpose')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <div class="form-text text-muted">
                        ...
                        <div class="row mb-3">
                            <div class="col-12">
                                ...
                                <input type="text" class="form-control @error('destination') is-invalid @enderror" name="destination"
                                       ...
                                       value="{{ old('destination', $request->destination) }}" required>
                                @error('destination')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <div class="form-text text-muted">
                        ...
                        <div class="row mb-3">
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('notes') is-invalid @enderror" name="notes" rows="2"
                                          placeholder="@if($type === 'vehicle') Contoh: Harap bensin terisi sebelum digunakan. @else Contoh: Sudah terinstall software yang diperlukan. @endif">{{ old('notes', $request->notes) }}</textarea>
                                @error('notes')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror>
                                <div class="form-text text-muted">
                        ...
                        <div class="d-flex justify-content-between gap-2 pt-3">
                            <a href="{{ route('hr.vehicle-asset.show', $request->id) }}" class="btn btn-outline-secondary">
    ...
    <script>
        ...
        function toggleFields() {
            ...
            if (requestType === 'vehicle') {
                ...
                licensePlateField.required = false;
                licensePlateField.disabled = false; // Bisa diedit untuk asset juga
                if (licensePlateLabel) {
                ...
                // Untuk edit, end_date selalu bisa diedit (tidak readonly)
                if (endDateField) {
                    endDateField.readOnly = false;
                    if (selectedItemType === 'Laptop' && endDateInfo) {
                        endDateInfo.textContent = 'Untuk laptop, masa pinjam maksimal 1 bulan';
                        endDateInfo.className = 'text-info';
                    } else if (endDateInfo) {
                        endDateInfo.textContent = '';
                        endDateInfo.className = 'text-muted';
                    }
        ...
        function setEndDateForLaptop() {
            ...
            // Untuk edit, tidak otomatis set end_date, hanya tampilkan info
            if (requestType === 'asset' && selectedItemType === 'Laptop' && startDateField.value) {
                ...
                if (endDate && endDateInfo) {
                    const diffTime = endDate - startDate;
                    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
                    if (diffDays <= 31) {
                        endDateInfo.textContent = `Masa pinjam: ${diffDays} hari (maksimal 1 bulan)`;
                        endDateInfo.className = 'text-info';
                    } else {
                        endDateInfo.textContent = `Masa pinjam: ${diffDays} hari (melebihi 1 bulan, tidak disarankan)`;
                        endDateInfo.className = 'text-warning';
                    }
                } else if (endDateInfo) {
                    endDateInfo.textContent = 'Untuk laptop, masa pinjam maksimal 1 bulan';
                    endDateInfo.className = 'text-info';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (startDateField && endDateField) {
                ...
                startDateField.addEventListener('change', function() {
                    ...
                    // Untuk edit, end_date selalu bisa diubah, hanya validasi
                    if (endDateField.value && new Date(endDateField.value) < startDate) {
                        // Tampilkan warning jika end_date lebih kecil dari start_date
                        if (endDateInfo) {
                            endDateInfo.textContent = 'Tanggal selesai tidak boleh lebih kecil dari tanggal mulai';
                            endDateInfo.className = 'text-danger';
                        }
                    } else {
                        // Update info untuk laptop
                ...
                // Update info saat end_date berubah
                ...
                form.addEventListener('submit', function(e) {
                    ...
                    Swal.fire({
                        ...
                        html: '<div class="text-left">' +
                              '<p>Apakah Anda yakin ingin menyimpan perubahan pada pengajuan ' + assetType + ' ini?</p>' +
