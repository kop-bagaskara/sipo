...
class ApprovalService
{
    ...
    protected function getAbsenceApprovalChain(EmployeeRequest $request, $divisiId)
    {
        ...
        //    Tapi SKIP level yang disabled di DivisiApprovalSetting
        $chain = [];
        ...
        foreach ($approvalFlow as $setting) {
            ...
            // 4. Cek di DivisiApprovalSetting apakah level ini enabled atau disabled
            //    Jika disabled (0), SKIP level ini
            //    Jika enabled, ambil dari pre-loaded users (batch query)
            //    KECUALIKAN jabatan 7 (KARYAWAN)
            $users = collect();

            if ($roleKey === 'spv_division') {
                // Cek apakah SPV enabled di DivisiApprovalSetting
    ...
    public function updateRequestStatus(EmployeeRequest $request)
    {
        ...
        if ($hasRejected) {
            ...
            // Urutan pengecekan: dari level tertinggi ke terendah (HR > Manager > Head > Supervisor)
