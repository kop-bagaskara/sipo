<?php $__env->startSection('title'); ?>
    Planning - <?php echo e(request()->get('processes', 'Semua Proses')); ?>
    ...
    <?php $__env->startSection('content'); ?>
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h3 class="text-themecolor">Planning Produksi</h3>
            </div>
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="javascript:void(0)">Penjadwalan</a></li>
                    <li class="breadcrumb-item active">Planning Produksi</li>
                </ol>
            </div>
        </div>
        <?php
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col">
                                ...
                                <p class="text-muted">Pilih item yang akan direncanakan untuk proses:
                                    <?php $__currentLoopData = $processList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $process): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <span class="process-badge <?php echo e(strtolower($process)); ?>"><?php echo e($process); ?></span>
                        ...
                        <div id="table-work-order" style="display: block;">
                            <div id="advanced_filter_wo">
                                <div class="row">
                                    <div class="col-md-4">
                                        <label id="filter_label_from"><?php echo e(in_array('PLONG', $processList) ? 'PLAN CETAK FINISH PADA - From' : 'SO Date - From'); ?></label>
                                        <input type="date" class="form-control" name="from_date_advanced_filter_wo"
                                            id="from_date_advanced_filter_wo">
                                    </div>
                                    <div class="col-md-4">
                                        <label id="filter_label_to"><?php echo e(in_array('PLONG', $processList) ? 'PLAN CETAK FINISH PADA - To' : 'SO Date - To'); ?></label>
                            ...
                            <table id="datatable-work-order" class="table table-hover nowrap">
                                <thead>
                                    <tr>
                                        ...
                                        <?php if(in_array('PLONG', $processList)): ?>
                                        <th>Jumlah PLONG</th>
                                        <th style="display: none;">Material Codes PLONG</th>
                                        <?php endif; ?>
                                        <th>WO DocNo</th>
                                        <th>SO DocNo</th>
                                        <th>Delivery Date</th>
            ...
            $(document).ready(function() {
                ...
                if (!processes) {
                    Swal.fire({
                        ...
                        window.location.href = '<?php echo e(route('mulai-proses.plan')); ?>';
                    });
                    return;
                }
                ...
                if (processes == 'PLONG') {
                    ...
                    // Ubah label filter untuk PLONG
                    $('#filter_label_from').text('PLAN CETAK FINISH PADA - From');
                    $('#filter_label_to').text('PLAN CETAK FINISH PADA - To');
                } else {
                    $('#table-work-order').show();
                    $('#table-work-order-cetak').hide();
                    // Label filter default untuk proses lain
                ...
                const table = $('#datatable-work-order').DataTable({
                    ...
                    ajax: {
                        url: "<?php echo e(route('wo-data.index')); ?>",
                        data: function(d) {
                    ...
                    columns: [{
                        ...
                        }].concat(processes === 'PLONG' ? [{
                            data: 'JumlahPlong',
                            name: 'JumlahPlong',
                            className: 'text-center',
                            render: function(data, type, row) {
                                // Untuk type 'display': return HTML badge (untuk tampilan)
                                // Untuk type 'data', 'type', 'filter', 'sort': return angka mentah (untuk data processing)
                                if (type === 'display') {
                                    // Data dari backend sudah dalam format HTML dengan tooltip
                                    return data || '<span class="badge badge-warning">0</span>';
                                } else {
                                    // Untuk type lain, return angka mentah dari row data
                                    // Jika data adalah HTML string, extract angka dari dalam badge
                                    if (typeof data === 'string' && data.includes('badge')) {
                                        // Extract angka dari HTML badge: <span...>2</span>
                                        const match = data.match(/>(\d+)</);
                                        return match ? parseInt(match[1]) : (row.JumlahPlong || 0);
                                    }
                                    // Jika sudah angka, return langsung
                                    return parseInt(data) || parseInt(row.JumlahPlong) || 0;
                                }
                            }
                        },
                        {
                            data: 'MaterialCodesPlong',
                            name: 'MaterialCodesPlong',
                            visible: false, // Kolom tersembunyi, hanya untuk data
                            orderable: false,
                            searchable: false
                            // Tidak perlu render function, biarkan DataTables handle array secara default
                        }] : []).concat([{
                            data: 'WODocNo',
                            name: 'WODocNo'
                        },
                    ...
                    columnDefs: processes === 'PLONG' ? [{
                        targets: 2, // Target kolom MaterialCodesPlong (index 2: checkbox=0, JumlahPlong=1, MaterialCodesPlong=2)
                        visible: false,
                        width: 0
                    }] : [],
                ...
                const tableCetak = $('#datatable-work-order-cetak').DataTable({
                    ...
                    deferLoading: 0, // biar kosong dulu sampai user klik Filter
                    ajax: {
                        url: "<?php echo e(route('hasil-cetak.index')); ?>",
                        data: function(d) {
                ...
                function updateSelection() {
                    ...
                    $('.select-wodocno:checked').each(function() {
                        ...
                        // Untuk PLONG: coba ambil MaterialCodesPlong dari kolom tersembunyi jika tidak ada di rowData
                        if (processes === 'PLONG' && (!rowData.MaterialCodesPlong || rowData.MaterialCodesPlong.length === 0)) {
                            // Coba ambil dari cell langsung
                            const rowIndex = table.row(row).index();
                            if (rowIndex !== null && rowIndex !== undefined) {
                                // MaterialCodesPlong adalah kolom index 2 (checkbox=0, JumlahPlong=1, MaterialCodesPlong=2)
                                const cellData = table.cell(rowIndex, 2).data();
                                if (cellData && cellData !== rowData.MaterialCodesPlong) {
                                    console.log('MaterialCodesPlong found in cell data:', cellData);
                                    rowData.MaterialCodesPlong = cellData;
                                }
                            }
                        }
                        ...
                        if (processes === 'PLONG') {
                            console.log('MaterialCodesPlong in rowData:', rowData?.MaterialCodesPlong);
                            console.log('MaterialCodesPlong type:', typeof rowData?.MaterialCodesPlong);
                        }
                        console.log('---');
                        ...
                        if (rowData) {
                            ...
                            const processedRowData = {
                                ...
                                // Untuk PLONG: simpan JumlahPlong dan MaterialCodesPlong untuk duplikasi nanti
                                JumlahPlong: processes === 'PLONG' ? (() => {
                                    // Ambil JumlahPlong dari rowData
                                    // Jika berupa HTML string (badge), extract angka dari dalamnya
                                    let jumlahPlong = 1;
                                    if (typeof rowData.JumlahPlong === 'string' && rowData.JumlahPlong.includes('badge')) {
                                        // Extract angka dari HTML badge: <span...>2</span>
                                        const match = rowData.JumlahPlong.match(/>(\d+)</);
                                        jumlahPlong = match ? parseInt(match[1]) : 1;
                                    } else {
                                        jumlahPlong = parseInt(rowData.JumlahPlong) || 1;
                                    }
                                    console.log('=== JumlahPlong Extraction ===');
                                    console.log('Raw JumlahPlong from rowData:', rowData.JumlahPlong);
                                    console.log('Parsed JumlahPlong:', jumlahPlong);
                                    console.log('Type of JumlahPlong:', typeof rowData.JumlahPlong);
                                    return jumlahPlong;
                                })() : 1,
                                MaterialCodesPlong: processes === 'PLONG' ? (() => {
                                    // Parse MaterialCodesPlong jika berupa JSON string atau array
                                    console.log('=== MaterialCodesPlong Extraction ===');
                                    console.log('Raw MaterialCodesPlong from rowData:', rowData.MaterialCodesPlong);
                                    console.log('Type of MaterialCodesPlong:', typeof rowData.MaterialCodesPlong);
                                    console.log('All rowData keys:', Object.keys(rowData));

                                    let materialCodes = rowData.MaterialCodesPlong || [];

                                    // Cek berbagai format yang mungkin
                                    if (materialCodes === null || materialCodes === undefined) {
                                        console.warn('MaterialCodesPlong is null/undefined');
                                        materialCodes = [];
                                    } else if (typeof materialCodes === 'string') {
                                        // Jika string, coba parse sebagai JSON
                                        if (materialCodes.trim() === '' || materialCodes === '[]') {
                                            materialCodes = [];
                                        } else {
                                            try {
                                                materialCodes = JSON.parse(materialCodes);
                                                console.log('Parsed MaterialCodesPlong from JSON:', materialCodes);
                                            } catch (e) {
                                                console.warn('Failed to parse MaterialCodesPlong as JSON:', e);
                                                // Coba split jika berupa comma-separated string
                                                if (materialCodes.includes(',')) {
                                                    materialCodes = materialCodes.split(',').map(s => s.trim()).filter(s => s);
                                                    console.log('Parsed MaterialCodesPlong from comma-separated:', materialCodes);
                                                } else {
                                                    materialCodes = [];
                                                }
                                            }
                                        }
                                    } else if (!Array.isArray(materialCodes)) {
                                        console.warn('MaterialCodesPlong is not an array, converting:', materialCodes);
                                        materialCodes = [];
                                    }

                                    const result = Array.isArray(materialCodes) ? materialCodes : [];
                                    console.log('Final MaterialCodesPlong array:', result);
                                    console.log('MaterialCodesPlong length:', result.length);
                                    if (result.length > 0) {
                                        console.log('MaterialCodesPlong items:', result);
                                    } else {
                                        console.warn('⚠️ MaterialCodesPlong is EMPTY! This will cause CodeItemBOM to be null.');
                                    }
                                    return result;
                                })() : []
                            };
                            ...
                            // Debug untuk PLONG
                            if (processes === 'PLONG') {
                                console.log('JumlahPlong:', processedRowData.JumlahPlong);
                                console.log('MaterialCodesPlong:', processedRowData.MaterialCodesPlong);
                                console.log('MaterialCodesPlong type:', typeof processedRowData.MaterialCodesPlong);
                                console.log('MaterialCodesPlong length:', Array.isArray(processedRowData.MaterialCodesPlong) ? processedRowData.MaterialCodesPlong.length : 'not an array');
                            }
                            console.log('---');
                ...
                $('#btnPreviewPlanning').on('click', async function() {
                    ...
                    for (let i = 0; i < selectedRows.length; i++) {
                        ...
                        try {
                            const response = await fetch('<?php echo e(route('get-machine-mapping')); ?>', {
                                method: 'POST',
                                headers: {
                    ...
                    fetch('<?php echo e(route('plan-first.submit')); ?>', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            ...
                            if (res.success === true) {
                                Swal.fire({
                                    ...
                                    window.location.href =
                                        '<?php echo e(route('process.plan-first-prd')); ?>';
                ...
                async function generatePreviewHtml(data) {
                    ...
                    try {
                        ...
                        const filteredAndSortedItems = filterAndSortItemsByProcess(planPerItemData,
                            selectedProcess);
                        ...
                        if (Array.isArray(planPerItem)) {
                            ...
                            Object.entries(planPerItem).forEach(([itemCode, plans], index) => {
                                ...
                                const filteredItem = {
                                    ...
                                    // Untuk PLONG: tambahkan CodeItemBOM dari plan data
                                    CodeItemBOM: plans[0].CodeItemBOM || plans[0].MaterialCodeBOM || plans[0].code_item_bom || null,
                                    MaterialCodeBOM: plans[0].CodeItemBOM || plans[0].MaterialCodeBOM || plans[0].code_item_bom || null
                                };
                ...
                async function generateIntegratedProcessTable(filteredItems, startTime, selectedProcess) {
                    ...
                    // Untuk PLONG: Reset global counter
                    const isPlongGlobalTable = selectedProcess === 'PLONG' || selectedProcess === 'PLG';
                    if (isPlongGlobalTable) {
                        window.globalTotalProcessTime = 0;
                    }
                    ...
                    async function calculateMachineTime(item, processType) {
                        try {
                            ...
                            const mappingResponse = await fetch('<?php echo e(route('get-machine-mapping')); ?>', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                            ...
                            const capacityResponse = await fetch('<?php echo e(route('get-machine-capacity')); ?>', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')
                    ...
                    // Untuk PLONG: Semua data di satu tabel global (tidak grouping per mesin)
                    // Untuk proses lain: Group items by machine
                    let itemsToProcess = [];
                    // isPlongGlobalTable sudah dideklarasikan di atas, tidak perlu dideklarasikan lagi

                    if (isPlongGlobalTable) {
                        // PLONG: Semua item langsung di satu tabel
                        itemsToProcess = [{ machine: 'ALL', items: filteredItems }];
                    } else {
                        // Proses lain: Group by machine
                        const machineGroups = {};
                    ...
                    // Generate table
                    // Untuk PLONG: Langsung buat satu tabel global untuk semua items (TIDAK grouping per mesin)
                    // Untuk proses lain: Loop per mesin dengan grouping
                    if (isPlongGlobalTable) {
                        // PLONG: Tabel global tanpa grouping mesin - SEMUA ITEMS LANGSUNG DI SATU TABEL
                        html += `
                        ...
                        // Loop SEMUA items untuk PLONG langsung (tanpa grouping, tanpa cek mesin, tanpa cek m_plg)
                        let groupTotalProcessTime = 0;
                        for (let itemIndex = 0; itemIndex < filteredItems.length; itemIndex++) {
                            const item = filteredItems[itemIndex];

                            // Process item untuk PLONG (sama seperti proses lain, tapi tanpa cek mapping mesin)
                            ...
                            let estHours = 0;
                            let machineName = 'PLONG'; // Default untuk PLONG
                            ...
                            if (item.WODocNo && item.WODocNo.startsWith('WOP')) {
                                ...
                                if (item.estHours && item.estHours > 0) {
                                    ...
                                    const machineCapacity = 1000; // Default untuk PLONG, tidak perlu cek mapping
                                    machineName = 'PLONG'; // Default machine name untuk PLONG
                            ...
                            // Untuk PLONG: Tidak perlu cek mapping mesin, langsung ready
                            const rowClass = '';
                            const statusDisplay = `<span class="badge badge-success">✓ Ready</span>`;
                            ...
                            // Untuk PLONG: Gunakan index global
                            const rowIdPrefix = `row-${itemIndex + 1}`;
                            const rowNumber = itemIndex + 1;
                            const itemMachine = machineName;

                            // Ambil Material Code BOM untuk PLONG
                            const materialCodeBOM = item.CodeItemBOM || item.MaterialCodeBOM || item.code_item_bom || '-';
                            ...
                            html += `
                                <tr class="${rowClass}">
                                    ...
                                    <td id="${rowIdPrefix}-materialcodebom">${materialCodeBOM}</td>
                                    ...
                                    <td class="text-center" id="${rowIdPrefix}-machine">${itemMachine}</td>
                                    <td class="text-center" id="${rowIdPrefix}-status">${statusDisplay}</td>
                                </tr>
                            `;

                            // Update waktu untuk item berikutnya (berlanjut global untuk PLONG)
                            currentTime = new Date(processEndTime);
                            groupTotalProcessTime += estHours;

                            // Akumulasi global untuk PLONG
                            if (typeof window.globalTotalProcessTime === 'undefined') {
                                window.globalTotalProcessTime = 0;
                            }
                            window.globalTotalProcessTime += estHours;

                            item.EndJam = processEndTime.toISOString();
                            console.log(`PLONG - EndJam saved for ${item.WODocNo}: ${item.EndJam}`);
                            console.log(`  Updated currentTime for next item: ${currentTime.toISOString()}`);
                            console.log(`  Global process time: ${window.globalTotalProcessTime || 0} hours`);
                        }

                        // Tutup tabel global PLONG dan tampilkan summary
                        let globalTotalItems = filteredItems.length;
                        let globalTotalTime = window.globalTotalProcessTime || 0;
                        ...
                        html += `
                            ...
                            <div class="mt-3">
                                <div class="alert alert-info">
                                    <h6><i class="fas fa-info-circle"></i> Summary Global PLONG</h6>
                                    <p class="mb-1"><strong>Total Items:</strong> ${globalTotalItems}</p>
                                    <p class="mb-1"><strong>Total Duration:</strong> ${globalTotalTime.toFixed(2)} jam</p>
                                    <p class="mb-0"><strong>Start Time:</strong> ${new Date(startDateInput?.value || new Date()).toLocaleDateString('id-ID')} 08:00:00</p>
                                </div>
                            </div>
                        `;

                        // Reset global counter untuk next render
                        window.globalTotalProcessTime = 0;
                    } else {
                        // Proses lain (CETAK, dll): Loop per mesin dengan grouping
                        for (let groupIndex = 0; groupIndex < itemsToProcess.length; groupIndex++) {
                            ...
                            // Cek apakah ada item tanpa mapping untuk tampilkan warning (hanya untuk proses selain PLONG)
                            ...
                            if (item.WODocNo && item.WODocNo.startsWith('WOP')) {
                                ...
                                if (item.estHours && item.estHours > 0) {
                                    ...
                                    // Untuk PLONG: Tidak perlu cek mapping mesin (m_plg), gunakan default
                                    // Untuk proses lain: Cek mapping mesin seperti biasa
                                    if (isPlongGlobalTable) {
                                        // PLONG: Gunakan default capacity, tidak perlu cek mapping
                                        machineCapacity = 1000; // Default capacity untuk PLONG
                                        machineName = 'PLONG'; // Default machine name
                                    } else {
                                        // Proses lain: Cek mapping mesin seperti biasa
                                        try {
                            ...
                            // Untuk PLONG: Skip cek mapping mesin dan machine unit
                            // Untuk proses lain: Cek machine unit seperti biasa
                            if (!isPlongGlobalTable) {
                                // get machine unit (hanya untuk proses selain PLONG)
                                var machineUnit = '';
                                $.ajax({
                                    url: '<?php echo e(route('get-machine-capacity')); ?>',
                                    type: 'POST',
                                    data: {
                                        machine: machine
                                    },
                                    success: function(data) {
                                        machineUnit = data.unit;
                                    }
                                });
                            }
                            ...
                            // Untuk PLONG: Tidak perlu cek mapping mesin, langsung ready
                            // Untuk proses lain: Cek mapping mesin seperti biasa
                            const hasNoMapping = isPlongGlobalTable ? false : (item.hasNoMapping || !machine || machine === 'CTK-001');
                            const rowClass = hasNoMapping ? 'table-danger' : '';
                            const statusDisplay = isPlongGlobalTable ?
                            ...
                            // Untuk PLONG: Gunakan index global, untuk proses lain: gunakan machineIndex
                            const rowIdPrefix = isPlongGlobalTable ? `row-${itemIndex + 1}` : `row-${groupIndex + 1}-${itemIndex + 1}`;
                            const rowNumber = isPlongGlobalTable ? (itemIndex + 1) : (itemIndex + 1);

                            // Ambil machine name
                            // Untuk PLONG: Gunakan default 'PLONG', untuk proses lain: ambil dari item atau mapping
                            const itemMachine = isPlongGlobalTable ? machineName : (item.machine || item.plans?.[0]?.Machine || 'CTK-001');
                            ...
                            html += `
                                <tr class="${rowClass}">
                                    ...
                                    <td class="text-center" id="${rowIdPrefix}-duration">${estHours.toFixed(2)} jam</td>
                                    <td class="text-center" id="${rowIdPrefix}-up">${item.Up || item.plans?.[0]?.UP || '1'}</td>
                                    ${isPlongGlobalTable ? `<td class="text-center" id="${rowIdPrefix}-machine">${itemMachine}</td>` : ''}
                                    <td class="text-center" id="${rowIdPrefix}-status">${statusDisplay}</td>
                                </tr>
                            `;

                            // Update waktu untuk item berikutnya
                            // Untuk PLONG: waktu berlanjut global, untuk proses lain: per mesin
                            currentTime = new Date(processEndTime);
                            lastEndTime = processEndTime;
                            groupTotalProcessTime += estHours;

                            // Untuk PLONG: akumulasi global, untuk proses lain: hanya untuk group ini
                            if (isPlongGlobalTable) {
                                if (typeof window.globalTotalProcessTime === 'undefined') {
                                    window.globalTotalProcessTime = 0;
                                }
                            ...
                            console.log('---');
                            } // 🔧 PERBAIKAN: Tutup for loop items (proses lain)
                        ...
                        } // 🔧 PERBAIKAN: Tutup for loop groups (proses lain)
                    } // 🔧 PERBAIKAN: Tutup if/else isPlongGlobalTable
                ...
                function generateItemView(sortedItems, currentGlobalTime) {
                    ...
                    const processOrder = ['CTK', 'PTG', 'TUM', 'EMB', 'PLG', 'KPS', 'STR', 'LEM', 'EPL', 'Finishing',
                        'Packing'
                    ];
...
<?php echo $__env->make('main.layouts.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\sjm\sipo_krisan\resources\views/main/process/planning.blade.php ENDPATH**/ ?>