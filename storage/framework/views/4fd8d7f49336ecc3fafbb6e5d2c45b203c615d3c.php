...
<?php $__env->startSection('content'); ?>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="btn-group mb-3" role="group" aria-label="View Toggle">
                        <a href="<?php echo e(route('process.plan-first-prd')); ?>" class="btn btn-outline-info">
                            <i class="mdi mdi-chart-gantt"></i> Timeline View
                        </button>
                        <a href="<?php echo e(route('process.plan-first-table')); ?>" class="btn btn-info">
    ...
    <!-- Combine Items Modal -->
    <div class="modal fade" id="combineItemsModal" tabindex="-1" role="dialog"
        aria-labelledby="combineItemsModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                <div class="modal-header bg-secondary text-white">
                    <h5 class="modal-title" id="combineItemsModalLabel">
                        <i class="mdi mdi-link-variant"></i> Gabungkan Item Lain
                        <span id="combine-machine-label" class="ml-2"></span>
                    </h5>
                    <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        <i class="mdi mdi-information-outline"></i>
                        <strong>Info:</strong> Pilih item-item yang ingin digabungkan. Data diambil dari tabel yang sedang ditampilkan.
                        <br><small>Item yang terpotong (split items) dengan quantity kecil biasanya cocok untuk digabungkan.</small>
                    </div>

                    <div class="row mb-3">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="combine-filter-quantity" class="form-label">
                                    <i class="mdi mdi-filter"></i> Filter Quantity Maksimal
                                </label>
                                <input type="number" class="form-control" id="combine-filter-quantity"
                                       placeholder="Kosongkan untuk menampilkan semua" min="0" step="1">
                                <small class="form-text text-muted">Hanya tampilkan item dengan quantity ≤ nilai ini</small>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="combine-filter-code" class="form-label">
                                    <i class="mdi mdi-magnify"></i> Filter Kode Item
                                </label>
                                <input type="text" class="form-control" id="combine-filter-code"
                    ...
                    <div class="table-responsive" style="max-height: 500px; overflow-y: auto;">
                        <table class="table table-sm table-bordered table-hover">
                            <thead class="thead-light sticky-top">
                                <tr>
                                    <th style="width: 50px;">
                                        <input type="checkbox" id="combine-select-all" title="Pilih Semua">
                                    </th>
                                    ...
                                    <th style="width: 150px;">Mulai Cetak</th>
                                    <th style="width: 150px;">Selesai Cetak</th>
                                    <th style="width: 100px;" class="text-center">Status</th>
                                </tr>
                            </thead>
                            <tbody id="combine-items-list">
                                <tr>
                    ...
                    <div class="mt-3">
                        <strong>Item Terpilih: </strong>
                        <span id="combine-selected-count" class="badge badge-primary">0</span>
                        <span class="text-muted ml-2">item</span>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">
                    ...
                    <button type="button" class="btn btn-primary" id="confirm-combine-items" disabled>
                        <i class="mdi mdi-check"></i> Gabungkan Item Terpilih
                    </button>
                </div>
            </div>
        </div>
...
<?php $__env->startSection('scripts'); ?>
    ...
    <script>
        ...
        const HistoryManager = {
            ...
            saveState(action = 'Unknown action') {
                ...
                planHistory.push({
                    action: action,
                    timestamp: new Date().toISOString(),
                    user: '<?php echo e(auth()->user()->name ?? "System"); ?>',
        ...
        // Store processed data per machine for combine items feature
        const machineProcessedData = {};
            ...
            async loadData() {
                try {
                    ...
                    const [machineRes, planRes, deptRes, shiftConfigRes] = await Promise.all([
                        fetch("<?php echo e(route('master.machine-cetak.data')); ?>").then(r => r.json()),
                        fetch("<?php echo e(route('plan.first.data')); ?>").then(r => r.json()),
                        fetch("<?php echo e(route('departments.list')); ?>").then(r => r.json()),
                        fetch("<?php echo e(route('process.machine-shift-config')); ?>").then(r => r.json())
                    ]);
            ...
            setupAddPlanManualEvents() {
                ...
                document.addEventListener('click', function(event) {
                    ...
                    if (shiftBtn) {
                        ...
                        // Save to database
                        fetch("<?php echo e(route('process.save-machine-shift-config')); ?>", {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
        ...
        const TableRenderer = {
            ...
            /**
             * Populate machine table directly without re-processing (for merge operation)
             */
            populateMachineTableDirect(machineCode, items) {
                ...
                tbody.innerHTML = '';
                console.log(`📊 populateMachineTableDirect: Rendering ${items.length} items directly for ${machineCode}`);
                ...
                // DON'T re-process, just render the items directly
                items.forEach((item, index) => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>${Utils.calculateDuration(item.start_jam, item.end_jam)} jam</td>
                        <td class="text-center">
                            <input type="number"
                        ...
                        <td class="text-center">
                            <select class="form-control form-control-sm status-select"
                                ...
                                <option value="NEW" ${item.job_order_status === 'NEW' ? 'selected' : ''}>NEW</option>
                                <option value="REPEAT" ${item.job_order_status === 'REPEAT' ? 'selected' : ''}>REPEAT</option>
                            </select>
                        </td>
                ...
                // Update machineProcessedData with current items
                machineProcessedData[machineCode] = items;

                // Load job orders for items
            ...
            populateMachineTable(machineCode, machineData) {
                ...
                // Store processed data for combine items feature
                // Filter out items with quantity = 0 or flag_status = 'MERGED' before storing
                const validProcessedData = processedData.filter(item => {
                    const qty = parseInt(item.quantity || 0);
                    const status = item.flag_status || '';
                    return qty > 0 && status !== 'MERGED';
                });
                machineProcessedData[machineCode] = validProcessedData;
        ...
        `;
            ...
            showMergeItemModal(itemId) {
                // First, try to find item from processedData (table view)
                let sourceItem = null;
                let machineCode = null;

                // Search in all machine processed data
                for (const [mc, processedData] of Object.entries(machineProcessedData)) {
                    const found = processedData.find(i => String(i.id) === String(itemId));
                    if (found) {
                        sourceItem = found;
                        machineCode = mc;
                        break;
                ...
                // If not found in processedData, try timelineData
                if (!sourceItem) {
                    sourceItem = timelineData.find(i => String(i.id) === String(itemId));
                    if (sourceItem) {
                        machineCode = sourceItem.code_machine;
                    }
                }

                if (!sourceItem) {
                    Utils.showToast('Item tidak ditemukan di tabel yang ditampilkan', 'danger');
                    console.error('Item not found:', itemId, 'Available in processedData:', Object.keys(machineProcessedData));
                    return;
                }

                // Get processed data from table view (not database) - this is the source of truth
                const processedData = machineProcessedData[machineCode] || [];

                // Find source item in processedData to get current state
                const sourceInProcessed = processedData.find(i => String(i.id) === String(itemId)) || sourceItem;

                // Find potential merge targets (same code_item, from table view)
                // Only use processedData - items that are actually visible in the table
                const mergeTargets = processedData.filter(item =>
                    String(item.id) !== String(itemId) &&
                    item.code_item === sourceInProcessed.code_item
                );

                // Also check other machines' processedData for items in other machines
                const otherMachineTargets = [];
                for (const [mc, otherProcessedData] of Object.entries(machineProcessedData)) {
                    if (mc !== machineCode) {
                        const targets = otherProcessedData.filter(item =>
                            String(item.id) !== String(itemId) &&
                            item.code_item === sourceInProcessed.code_item
                        );
                        otherMachineTargets.push(...targets);
                    }
                }

                // Combine both lists - all from table view, not database
                const allTargets = [...mergeTargets, ...otherMachineTargets];

                // Populate source item info (use sourceInProcessed for accurate data)
                document.getElementById('merge-source-info').innerHTML = `
                    <strong>${sourceInProcessed.code_item}</strong> - ${sourceInProcessed.material_name || '-'}<br>
                    Mesin: ${machineCode}<br>
                    Quantity: ${parseInt(sourceInProcessed.quantity || 0).toLocaleString()} PCS
                    ${sourceInProcessed.id && (String(sourceInProcessed.id).includes('_split_') || String(sourceInProcessed.id).includes('_shift')) ?
                        '<br><small class="text-muted">(Split Item)</small>' : ''}
                `;
                ...
                if (allTargets.length === 0) {
                    document.getElementById('merge-target-list').innerHTML = `
                        <tr>
                            <td colspan="5" class="text-center text-muted py-3">
                                <i class="mdi mdi-information-outline"></i>
                                Tidak ada item dengan kode yang sama untuk digabungkan
                            </td>
                        </tr>
                    `;
                    ...
                    allTargets.forEach((targetItem, index) => {
                        const radioId = `merge-target-${targetItem.id}`;
                        const row = document.createElement('tr');
                        const isSameMachine = targetItem.code_machine === machineCode;
                        row.innerHTML = `
                            <td class="text-center">
                            ...
                            <td>
                                <label for="${radioId}" class="mb-0 w-100 cursor-pointer">
                                    ...
                                    ${targetItem.id && (String(targetItem.id).includes('_split_') || String(targetItem.id).includes('_shift')) ?
                                        '<br><small class="text-muted">(Split Item)</small>' : ''}
                                </label>
                            </td>
                            <td>
                                <label for="${radioId}" class="mb-0 w-100 cursor-pointer">
                                    ...
                                    ${isSameMachine ? '<br><small class="text-info">(Mesin Sama)</small>' : ''}
                                </label>
                            </td>
                            <td>
                                <label for="${radioId}" class="mb-0 w-100 cursor-pointer">
            ...
            updateMergeTargetSelection() {
                ...
                if (selectedTarget) {
                    // Find target item from processedData (table view)
                    let targetItem = null;
                    for (const processedData of Object.values(machineProcessedData)) {
                        const found = processedData.find(i => String(i.id) === String(selectedTarget.value));
                        if (found) {
                            targetItem = found;
                            break;
                        }
                    ...
                    // Fallback to timelineData if not found in processedData
                    if (!targetItem) {
                        targetItem = timelineData.find(i => String(i.id) === String(selectedTarget.value));
                    }
            ...
            canMergeItem(item) {
                // Check if there are other items with same code_item (in same or different machines)
                // This includes duplicate items and split items
            ...
            /**
             * Merge items from table view (processedData)
             */
            mergeItemsFromView(sourceItem, targetItem, sourceMachineCode, targetMachineCode) {
                console.log('🔗 Merging items from view:', {
                    source: sourceItem.id,
                    sourceCode: sourceItem.code_item,
                    sourceQty: sourceItem.quantity,
                    target: targetItem.id,
                    targetCode: targetItem.code_item,
                    targetQty: targetItem.quantity,
                    sourceMachine: sourceMachineCode,
                    targetMachine: targetMachineCode
                });

                // Calculate new quantity
                const sourceQty = parseInt(sourceItem.quantity || 0);
                const targetQty = parseInt(targetItem.quantity || 0);
                const newQuantity = sourceQty + targetQty;

                console.log(`📊 Quantity calculation: ${sourceQty} + ${targetQty} = ${newQuantity}`);

                // Update target item quantity
                targetItem.quantity = newQuantity;

                // Recalculate end_jam based on new quantity and capacity
                if (targetItem.start_jam && targetItem.up_cetak) {
                    const capacity = parseFloat(targetItem.up_cetak || targetItem.capacity || 10000);
                    const durationHours = newQuantity / capacity;
                    const startTime = new Date(targetItem.start_jam);
                    const endTime = new Date(startTime);
                    endTime.setTime(startTime.getTime() + (durationHours * 60 * 60 * 1000));
                    targetItem.end_jam = endTime.toISOString();
                }

                // Remove source item from processedData - mark for deletion
                sourceItem._deleted = true;

                if (machineProcessedData[sourceMachineCode]) {
                    const beforeCount = machineProcessedData[sourceMachineCode].length;
                    // Filter out source item by ID
                    machineProcessedData[sourceMachineCode] = machineProcessedData[sourceMachineCode].filter(
                        i => String(i.id) !== String(sourceItem.id)
                    );
                    console.log(`✅ Source item removed from processedData. Before: ${beforeCount}, After: ${machineProcessedData[sourceMachineCode].length}`);
                } else {
                    console.warn(`⚠️ Machine processedData not found: ${sourceMachineCode}`);
                }

                // Update target item in processedData
                if (machineProcessedData[targetMachineCode]) {
                    const index = machineProcessedData[targetMachineCode].findIndex(
                        i => String(i.id) === String(targetItem.id)
                    );
                    if (index !== -1) {
                        // Update the item in place
                        Object.assign(machineProcessedData[targetMachineCode][index], targetItem);
                        console.log('✅ Target item updated in processedData:', machineProcessedData[targetMachineCode][index]);
                    } else {
                        // Target item not found - add it
                        console.warn('⚠️ Target item not found in processedData, adding it');
                        machineProcessedData[targetMachineCode].push({ ...targetItem });
                    }
                } else {
                    // Machine data doesn't exist - create it
                    console.warn('⚠️ Machine processedData not found, creating it');
                    machineProcessedData[targetMachineCode] = [{ ...targetItem }];
                }

                // Also update timelineData if items exist there
                // Remove source item from timelineData (both split and original items should be removed after merge)
                const beforeTimelineCount = timelineData.length;
                // Filter out source item by ID
                const newTimelineData = timelineData.filter(i => String(i.id) !== String(sourceItem.id));
                if (newTimelineData.length < beforeTimelineCount) {
                    // Clear and repopulate timelineData
                    timelineData.length = 0;
                    newTimelineData.forEach(item => timelineData.push(item));
                    console.log(`✅ Source item removed from timelineData. Before: ${beforeTimelineCount}, After: ${timelineData.length}`);
                } else {
                    console.warn(`⚠️ Source item not found in timelineData: ${sourceItem.id}`);
                }

                // Update target item in timelineData
                const targetIndex = timelineData.findIndex(i => String(i.id) === String(targetItem.id));
                if (targetIndex !== -1) {
                    timelineData[targetIndex] = { ...timelineData[targetIndex], ...targetItem };
                } else {
                    // Target item not in timelineData (might be split item) - add it
                    timelineData.push({ ...targetItem });
                }

                // Re-render tables for affected machines
                // Use processedData directly (from view), not filteredData from timelineData
                // Filter out items with quantity = 0 or flag_status = 'MERGED'
                ...
                if (sourceMachineCode === targetMachineCode) {
                    // Same machine - IMPORTANT: machineProcessedData already has source removed
                    // Just get the current data and re-render directly without any processing
                    const currentData = machineProcessedData[sourceMachineCode] || [];
                    console.log(`📋 Current machineProcessedData count: ${currentData.length}`, currentData.map(i => ({id: i.id, code: i.code_item, qty: i.quantity})));
                    ...
                    console.log(`🔄 Re-rendering with ${sortedData.length} items (source should already be removed)`);

                    // CRITICAL: Pass skipProcessing flag to prevent re-processing
                    TableRenderer.populateMachineTableDirect(sourceMachineCode, sortedData);
                    TableRenderer.syncMachineTimes(sourceMachineCode);
                } else {
                    // Different machines - render both using processedData directly
                    const sourceMachineData = machineProcessedData[sourceMachineCode] || [];
                    const sortedSourceData = [...sourceMachineData].sort((a, b) => {
                        if (!a.start_jam || !b.start_jam) return 0;
                        return new Date(a.start_jam) - new Date(b.start_jam);
                    });
                    console.log(`🔄 Re-rendering source machine ${sourceMachineCode} with ${sortedSourceData.length} items`);
                    TableRenderer.populateMachineTableDirect(sourceMachineCode, sortedSourceData);
                    TableRenderer.syncMachineTimes(sourceMachineCode);

                    const targetMachineData = machineProcessedData[targetMachineCode] || [];
                    const sortedTargetData = [...targetMachineData].sort((a, b) => {
                        if (!a.start_jam || !b.start_jam) return 0;
                        return new Date(a.start_jam) - new Date(b.start_jam);
                    });
                    console.log(`🔄 Re-rendering target machine ${targetMachineCode} with ${sortedTargetData.length} items`);
                    TableRenderer.populateMachineTableDirect(targetMachineCode, sortedTargetData);
                    TableRenderer.syncMachineTimes(targetMachineCode);
                }

                Utils.showToast(
                    `✅ Item berhasil digabungkan! Quantity baru: ${newQuantity.toLocaleString()} pcs`,
                    'success'
                );
            },
            ...
            setupMaterialNameAutoFill() {
                ...
                newCodeItemInput.addEventListener('blur', function() {
                    ...
                    $.ajax({
                        url: '<?php echo e(route("process.search-materials")); ?>',
                        method: 'GET',
                        data: {
                            q: materialCode,
            ...
            /**
             * Show Combine Items Modal
             */
            showCombineItemsModal(machineCode) {
                console.log('🔗 Showing Combine Items Modal for machine:', machineCode);
                ...
                // Set machine label
                document.getElementById('combine-machine-label').textContent = `${machineCode} - ${machineName}`;

                // Get processed data for this machine (from table view, not database)
                const processedData = machineProcessedData[machineCode] || [];

                if (processedData.length === 0) {
                    document.getElementById('combine-items-list').innerHTML = `
                        <tr>
                            <td colspan="9" class="text-center text-muted py-4">
                                <i class="mdi mdi-information"></i> Tidak ada data untuk mesin ini
                            </td>
                        </tr>
                    `;
                    $('#combineItemsModal').modal('show');
                    return;
                }

                // Store machine code in modal
                document.getElementById('combineItemsModal').dataset.machineCode = machineCode;

                // Populate items list
                this.populateCombineItemsList(processedData);

                // Setup filter handlers
                this.setupCombineItemsFilters(processedData);

                // Setup select all checkbox
                document.getElementById('combine-select-all').addEventListener('change', function(e) {
                    const checkboxes = document.querySelectorAll('.combine-item-checkbox');
                    checkboxes.forEach(cb => cb.checked = e.target.checked);
                    TableRenderer.updateCombineSelectedCount();
                });

                // Setup confirm button
                document.getElementById('confirm-combine-items').onclick = () => {
                    this.combineSelectedItems(machineCode);
                };
            ...
            /**
             * Populate combine items list
             */
            populateCombineItemsList(items, filterQuantity = null, filterCode = '') {
                const tbody = document.getElementById('combine-items-list');
                tbody.innerHTML = '';
                ...
                if (!items || items.length === 0) {
                    tbody.innerHTML = `
                        <tr>
                            <td colspan="9" class="text-center text-muted py-4">
                                <i class="mdi mdi-information"></i> Tidak ada data
                            </td>
                        </tr>
                    `;
                    return;
                ...
                if (filterQuantity !== null && filterQuantity !== '') {
                    filteredItems = filteredItems.filter(item => {
                        const qty = parseInt(item.quantity || 0);
                        return qty <= parseInt(filterQuantity);
                    });
                }
                if (filterCode) {
                    const codeLower = filterCode.toLowerCase();
                    filteredItems = filteredItems.filter(item => {
                        const itemCode = (item.code_item || '').toLowerCase();
                        return itemCode.includes(codeLower);
                    });
                }
                ...
                if (filteredItems.length === 0) {
                    tbody.innerHTML = `
                        <tr>
                            <td colspan="9" class="text-center text-muted py-4">
                                <i class="mdi mdi-filter-remove"></i> Tidak ada item yang sesuai filter
                            </td>
                        </tr>
                    `;
                    return;
                ...
                filteredItems.forEach((item, index) => {
                    const row = document.createElement('tr');
                    const isSplit = item.id && (String(item.id).includes('_split_') || String(item.id).includes('_shift'));
                    ...
                    row.innerHTML = `
                        <td class="text-center">
                            <input type="checkbox" class="combine-item-checkbox"
                                   data-item-id="${item.id}"
                                   data-quantity="${quantity}"
                                   onchange="TableRenderer.updateCombineSelectedCount()">
                        </td>
                        <td class="text-center">${index + 1}</td>
                        <td>
                            <strong>${item.code_item || '-'}</strong>
                            ${isSplit ? '<br><small class="text-muted">(Split Item)</small>' : ''}
                        </td>
                        <td>${item.material_name || '-'}</td>
                        <td>${item.wo_docno || '-'}</td>
                        <td class="text-center">
                            <span class="badge ${quantity < 100 ? 'badge-warning' : 'badge-info'}">
            ...
            /**
             * Setup combine items filters
             */
            setupCombineItemsFilters(allItems) {
                const quantityFilter = document.getElementById('combine-filter-quantity');
                const codeFilter = document.getElementById('combine-filter-code');

                const applyFilters = () => {
                    const filterQty = quantityFilter.value;
                    const filterCode = codeFilter.value.trim();
                    this.populateCombineItemsList(allItems, filterQty || null, filterCode);
                };

                quantityFilter.oninput = applyFilters;
            ...
            updateCombineSelectedCount() {
                const selected = document.querySelectorAll('.combine-item-checkbox:checked');
                const count = selected.length;
                document.getElementById('combine-selected-count').textContent = count;
                document.getElementById('confirm-combine-items').disabled = count < 2;
            },

            /**
             * Combine selected items
             */
            combineSelectedItems(machineCode) {
                const selected = document.querySelectorAll('.combine-item-checkbox:checked');

                if (selected.length < 2) {
                    Utils.showToast('❌ Pilih minimal 2 item untuk digabungkan', 'warning');
                    return;
                }

                const selectedIds = Array.from(selected).map(cb => cb.dataset.itemId);
                const selectedQuantities = Array.from(selected).map(cb => parseInt(cb.dataset.quantity || 0));
                const totalQuantity = selectedQuantities.reduce((sum, qty) => sum + qty, 0);

                // Get processed data
                const processedData = machineProcessedData[machineCode] || [];
                const selectedItems = processedData.filter(item => selectedIds.includes(String(item.id)));

                if (selectedItems.length < 2) {
                    Utils.showToast('❌ Item yang dipilih tidak ditemukan', 'error');
                    return;
                }

                // Sort by start_jam to get the earliest item as base
                selectedItems.sort((a, b) => {
                    if (!a.start_jam) return 1;
                    if (!b.start_jam) return -1;
                    return new Date(a.start_jam) - new Date(b.start_jam);
                });
                ...
                // Confirm action
                const confirmMsg = `Gabungkan ${selectedItems.length} item?\n\n` +
                    `Item Base: ${baseItem.code_item} (${baseItem.quantity} pcs)\n` +
                    `Item Lain: ${otherItems.map(i => `${i.code_item} (${i.quantity} pcs)`).join(', ')}\n\n` +
                    `Total Quantity: ${totalQuantity.toLocaleString()} pcs\n\n` +
                    `Item lain akan dihapus dan quantity akan ditambahkan ke item base.`;
                ...
                // Calculate new end_jam based on total quantity and capacity
                if (baseItem.start_jam && baseItem.up_cetak) {
                    const capacity = parseFloat(baseItem.up_cetak || baseItem.capacity || 10000);
                    const durationHours = totalQuantity / capacity;
                    const startTime = new Date(baseItem.start_jam);
                    const endTime = new Date(startTime);
                    endTime.setTime(startTime.getTime() + (durationHours * 60 * 60 * 1000));
                    baseItem.end_jam = endTime.toISOString();
                }

                // Remove other items from processed data
                const updatedData = processedData.filter(item => {
                    const itemId = String(item.id);
                    // Keep base item, remove others
                    if (itemId === String(baseItem.id)) {
                        return true;
                    }
                ...
                // Update stored data
                machineProcessedData[machineCode] = updatedData;

                // Update timelineData (remove other items, update base item)
                otherItems.forEach(item => {
                    const index = timelineData.findIndex(t => String(t.id) === String(item.id));
                    if (index !== -1) {
                        timelineData.splice(index, 1);
                    }
                });

                // Update base item in timelineData
                const baseIndex = timelineData.findIndex(t => String(t.id) === String(baseItem.id));
                if (baseIndex !== -1) {
                    timelineData[baseIndex] = { ...timelineData[baseIndex], ...baseItem };
                }
                ...
                // Close modal
                $('#combineItemsModal').modal('hide');

                Utils.showToast(`✅ ${selectedItems.length} item berhasil digabungkan. Total quantity: ${totalQuantity.toLocaleString()} pcs`, 'success');
            },
        ...
        document.getElementById('confirm-merge-btn').addEventListener('click', function() {
            ...
            // Find items from processedData (table view) first
            let sourceItem = null;
            let targetItem = null;
            let sourceMachineCode = null;
            let targetMachineCode = null;

            // Search in processedData
            for (const [machineCode, processedData] of Object.entries(machineProcessedData)) {
                if (!sourceItem) {
                    const found = processedData.find(i => String(i.id) === String(sourceItemId));
                    if (found) {
                        sourceItem = found;
                ...
                if (!targetItem) {
                    const found = processedData.find(i => String(i.id) === String(targetItemId));
                    if (found) {
                        targetItem = found;
                        targetMachineCode = machineCode;
                    }
                }
            ...
            // Fallback to timelineData if not found in processedData
            if (!sourceItem) {
                sourceItem = timelineData.find(i => String(i.id) === String(sourceItemId));
                if (sourceItem) sourceMachineCode = sourceItem.code_machine;
            }
            if (!targetItem) {
                targetItem = timelineData.find(i => String(i.id) === String(targetItemId));
                if (targetItem) targetMachineCode = targetItem.code_machine;
            }

            if (!sourceItem || !targetItem) {
                Utils.showToast('Item tidak ditemukan di tabel yang ditampilkan', 'error');
                return;
            }
            ...
            // Execute merge using processedData
            TableRenderer.mergeItemsFromView(sourceItem, targetItem, sourceMachineCode, targetMachineCode);
        });
        ...
        async function savePlanUpdate() {
            ...
            try {
                ...
                // Prepare request URL
                const url = '<?php echo e(route('process.save-plan-update')); ?>';
...
<?php echo $__env->make('main.layouts.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\sjm\sipo_krisan\resources\views/main/process/timeline-table.blade.php ENDPATH**/ ?>